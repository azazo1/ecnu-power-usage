<template>
    <div class="flex h-screen w-screen bg-green-50 text-gray-700 font-sans selection:bg-emerald-200">
        <aside class="w-64 bg-white border-r border-green-100 flex flex-col shadow-sm z-20">
            <div class="p-6">
            ...
            <nav class="flex-1 px-4 space-y-2">
                ...
                " class="w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200" :class="currentTab === 'records'
                ...
                <button @click="currentTab = 'archives'"
                    class="w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200" :class="currentTab === 'archives'
                        ? 'bg-emerald-100 text-emerald-800 font-semibold shadow-inner'
                        : 'hover:bg-gray-50 text-gray-600'
                        ">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <main class="flex-1 p-4 overflow-hidden relative">
            <transition name="fade" mode="out-in">
                ...
                <div v-else-if="currentTab === 'archives' && !selectedArchive"
                    ...
                    <div class="p-4 border-b border-emerald-50 flex justify-between items-center">
                        <h2 class="text-lg font-semibold text-emerald-800">
                            归档列表
                        </h2>
                        <button @click="refreshArchives"
                            class="p-2 text-emerald-600 hover:bg-emerald-50 rounded-lg transition" title="刷新">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 overflow-auto">
                        <div v-for="arc in archiveList" :key="arc.id" @click="openArchive(arc)"
                            ...
                            <div class="flex items-center gap-3 mb-2">
                                <div class="bg-white p-2 rounded-lg text-emerald-500">
                                ...
                                <div>
                                    <h3 class="font-bold text-gray-800 group-hover:text-emerald-800">
                                        {{ arc.name }}
                                    </h3>
                            ...
                            <div class="text-right text-xs text-emerald-600 font-medium">
                                点击查看详情 &rarr;
                            </div>
                        </div>
                    </div>
